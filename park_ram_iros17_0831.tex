...
\begin{align}
&\mathcal{L}(u_t) := P\lbrace \bm{y}_t\mid x_{0:t-1},\bm{y}_{1:t-1},u_t,\mathcal{F}\rbrace
\label{cost}
%&= \int_{\mathcal{Z}} 
%	f_{\bm{y}_{t} \mid
%	\bm{z},b_0,
...
\rood{which depends on a parameter vector, $u_t$, the control policy of $m$ robots between $t-1$ and $t$ depends on  the observations up to time $t-1$, $\bm{y}_{1:t-1}$, and robots' trajectories prior to time $t$, $x_{0:t-1}$ and the index set of faulty robots which are unknown to others.} Also, we note here that the target state has been marginalized out.
\rood{If we let $\mathcal{L}^+(u_t)$ be the likelihood of reliable sensor measurements, which will be discussed in Section \ref{sec:sec3}, then} the objective of \rood{our} environmental mapping problem is to obtain a sequence of optimal control policies $(u_1^{\star},u_2^{\star},\dots)$, each solves:
\begin{equation}
