...
Rather than rely on the information gradient, others have employed algorithm that uses information diffusion through communication network for environmental modeling \cite{lynch2008decentralized}.  By utilizing the Average Consensus filter to share information among the robots in the network, this approach is scalable to large numbers of agents, is fully decentralized, and can even work under a switching network topology as long as the network is connected; however the approach is not spatially distributed and require additional connectivity maintenance algorithm (see, e.g., \cite{yang2010decentralized}) to guarantee its convergence.
