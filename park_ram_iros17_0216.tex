...
Though such environmental mapping tasks can be done efficiently via dispatching a group of autonomous agents, it is typically undertaken by humans due to the lack of formal methods that are able to guarantee \Ram{need details here about what is the missing piece for existing methods}.
...
Both simulation results and real-world validation are presented to demonstrate the effectiveness of the proposed methods.
\end{abstract}
...
Despite these numerous applications and the potential of MSNs to resolve environmental mapping, the lack of \Ram{explain what is missing specifically now} has meant that such environmental mapping has relied predominantly on human-controlled efforts. 
...
In this latter instance for example,  the Bayesian approach led to the development of an autonomous search algorithm that maximized information gain to find a diffusive source\cite{ristic2010information} \Ram{why is this paper in particular relevant and what is the missing gap here between what they are proposing and what you are proposing in this paper??}.
...
first, a probabilistic sensor model that incorporates joint target detection and spatial distribution estimation by a group of mobile sensors while capturing a key characteristic of the target detection task--the probability of seeing a target, monotonically decreases as a function of the distance between the sensor and the target, which is atypical of range sensors \Ram{I don't understand what you mean by: atypical of range sensors.}. 
...
The effectiveness of this deployment and belief update approach is evaluated via simulation and real-world experimentation in Sections \ref{sec:sec7} and \ref{sec:sec8}, respectively.
...
Let the $m$-tuple $\bm{y}_t=(\bm{y}_t^1,\dots,\bm{y}_t^m)$ be the observations recorded by $m$ robots at time step $t$ where $\bm{y}_t^i$ denotes the observation made by the $i^{\textup{th}}$ sensor, and let the set $\bm{y}_{1:t}\coloneqq \lbrace \bm{y}_1,\dots,\bm{y}_t \rbrace$ denote observations made by $m$ robots up to time $t$. \Ram{it may make sense for the subsequent development for you to mention that $y$ is a function of $z$ and $x$.}
...
Let $b_t$ represent a \emph{belief}, the posterior probability distribution over the target state space at time $t \in \mathbb{Z}^{+}$ \Ram{Is $b_t = p(z \mid x_{0:t},y_{0:t}, b_0)$}. 
Each belief depends on initial belief, $b_0$, the set of robot configurations, $\bm{x}_{0:t}$, and observations up to this point, $\bm{y}_{1:t-1}$.
...
\end{align*}}}where $t \in \mathbb{N}$ \Ram{I still do not understand this previous equation. Here are my questions: 1) is it $b_0(z)$ or $b_0$ 2) is it $x_{0:t}$ or $x_{0:t-1}$? }.
Due to our sensor model (this is discussed in the next section), the observation $\bm{y}_{t}$ is conditionally independent of $b_0(z)$, $\bm{y}_{1:t-1}$, and $x_{0:t-1}$ when it is conditioned on $\bm{z}$ and $x_t$ \Ram{again is this supposed to be $b_0(z)$ or $b_0$?}. 
...
b_0(z).
\end{align}
We assume that $m$ robots share their beliefs \Ram{so you are assuming full information sharing?}. 
...
% \end{equation}
Note that two random vectors $\bm{y}_B^i$ and $\bm{y}_I^i$ are \emph{independent}\footnote{This assessment is based on the underlying assumption that the detection event and sensor measurement event are independent to each other \Ram{is this a valid assumption?}.} when conditioned on $x_t,\bm{z}$ so that joint PDF can be computed as:
\begin{equation}
...
Due to the limitation of the effective sensing range found on physical range sensors, we consider a partitioned-based strategy where the workspace is partitioned into $m$ disjoint regions, and each robot is assigned to a region where it confines its detections \Ram{just to be clear we are only going to consider the fully partitioned strategy in this paper?}.
...
\begin{definition}[An Order$-k$ Voronoi Partition \cite{shamos1975closest}]
	...
	The \emph{order-$k$ Voronoi partition of $\mathcal{Q}$ based on $x$} is the collection of regions that partitions $\mathcal{Q}$ where each region is associated with the $k$ nearest points in $x$.
	\label{orderk}
\end{definition}
We also define another bijection ${}^kG^{\star}$ that maps a region to a set of $k$ \emph{nearest} points \Ram{what set of points?}. 
...
In fact, the order-$k$ diagram \Ram{what is a diagram?} is the optimal workspace partition which minimizes $H$ for each choice of $x$ and $k$:
...
\label{sec:sec6}
We consider the Particle Filtering approach to reduce the complexity of the target map evolution. 
...
\textit{Remark:} as discussed in the previous studies (see, e.g., \cite{crisan2002survey}), our particle filter uses standard re-sampling scheme such that the convergence of the mean square error toward zero is ensured, and the convergence rate is $1/N_2$ for all $q \in \mathcal{Q}$. 
