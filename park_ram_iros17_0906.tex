...
\begin{align*}
\frac{\partial\mathcal{L}(x_t)}{\partial x_i^t} 
&= -\sum_{j:W_t^j \in {}^kG^{-1}(x_t^i)} \int_{W_t^j}
\frac{\partial h(\left\| q-x_t^i \right\|)}{\partial x_t^i} \\
&\,\,\,\,\,\,\, \times \prod_{l:x_{t}^l \in {}^kG(W_t^j),l\neq i, l \notin \mathcal{F}} \left(1- h \left(\left\|
...
%\includegraphics[width=1.57in]{figure/final_config_order1_fault}
	...
	\caption{One-time deployment with non-robust (left) and robust ($k=2$) (right) overlay-ed with map at $t=1$ (lines: gradient descent flow, stars: positions of robots, polygons: partition)}
	\label{fig:fig2}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=2.7in]{figure/cost_comp1}
	%\includegraphics[width=1.57in]{figure/final_config_order1_fault}
	%	\includegraphics[width=3.4in]{figure/init_10_deploy_cmd2}
...
%	\caption{$k=10$}
%\end{subfigure}
	\caption{Belief propagation with our robust deployment strategy where $k=2$, $r_{\textup{eff}} = 0.3$, $\Sigma_B = 0.04\mathbf{I}$.}
	\label{fig:fig3}
\end{figure*}
...
\begin{figure}
	\centering
	\includegraphics[width=2.7in]{figure/cost_comp0}
...
As can be seen from Fig \ref{fig:fig6} and Fig. \ref{fig:fig7}, the map retrieved by the proposed method $k=2$ is more robust to the sensor failure compared to that obtained with the non-robust method. In Fig \ref{fig:fig6}(right) the unmapped area in the bottom-middle is owing to the limited sensing range.
Due to its fully decentralized nature, it is not surprising to see from this example that the non-robust method works poorly even with a single sensor failures. 

\begin{figure}
	\centering
	\includegraphics[width=1.55in]{figure/order1_conf}
	\includegraphics[width=1.55in]{figure/order2_conf}
	%\includegraphics[width=1.57in]{figure/final_config_order1_fault}
	%	\includegraphics[width=3.4in]{figure/init_10_deploy_cmd2}
	\caption{The belief at $t=10$ with non-robust method (left) and robust method (right). Also showing robot configurations (stars, where red stars are robots whose sensors have failed), effective sensing range (dashed line), and partition (solid lines).
	}
	\label{fig:fig6}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=2.7in]{figure/cost_comp000}
...
I WILL TRY TO INCLUDE A FIGURE WITH MORE THAN 100 ROBOTS...
